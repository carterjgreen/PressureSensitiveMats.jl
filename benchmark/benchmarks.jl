...
SUITE["motion"]["Soleimani"] = @benchmarkable move_detect(Solei(),
    $single,
    min_samples = 10)

# Occupancy
SUITE["occupancy"]["regular"] = @benchmarkable occupancy_detection($multi)
SUITE["occupancy"]["dist method"] = @benchmarkable occupancy_detection($multi,
    max_dist = true)

SUITE["occupancy"]["two days regular"] = @benchmarkable occupancy_detection($two_days)
SUITE["occupancy"]["two days dist method"] = @benchmarkable occupancy_detection($two_days,
    max_dist = true)
# Combiners
SUITE["combiners"]["SNR-MAX"] = @benchmarkable combiner(SNR_MAX(), $multi)
SUITE["combiners"]["Pearson Correlation Coefficient"] = @benchmarkable combiner(PCC(),
    $multi)
SUITE["combiners"]["Pearson Correlation Coefficient v2"] = @benchmarkable combiner(PCC2(),
    $multi)
